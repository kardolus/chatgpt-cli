...
| `max_retries`            | Number of times to retry a request that fails with a connection error, `429`, or `5xx` status. A positive `Retry-After` header is honored; otherwise exponential backoff with jitter is used. `0` keeps the built-in default of 3; a negative value disables retries. Retries only re-establish the request, never mid-stream. Note: a retried completion `POST` can be processed twice server-side if the first response was lost after the request was received (same trade-off the official SDKs make). | `3`                       |
| `retry_base_delay_ms`    | Base delay in milliseconds for exponential backoff between retries (ignored when the server sends a `Retry-After` header).                                                                             | `500`                     |
...
| `url`                    | The base URL for the API — **origin only** (scheme + host). The `*_path` values are appended to it, so don't include `/v1` here or requests will 404 with a doubled prefix (e.g. use `url: https://my-host` + `completions_path: /v1/chat/completions`). | 'https://api.openai.com'       |
